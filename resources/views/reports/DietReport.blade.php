    ...
    <body>
        ...
        <main id="main" class="main">
            <div class="container-fluid">
                <h4 class="d-flex justify-content-center py-1 contitle">Diet Report</h4>                                  
            </div>
            <div class="row mt-4 mx-3">
                <ul class="nav nav-pills mb-3 nav-fill" id="pills-tab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active phtab" id="pills-opdiet-tab" data-bs-toggle="pill" data-bs-target="#pills-opdiet" type="button" role="tab" aria-controls="pills-home" aria-selected="true">OP Diet Report</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link phtab" id="pills-ipdiet-tab" data-bs-toggle="pill" data-bs-target="#pills-ipdiet" type="button" role="tab" aria-controls="pills-profile" aria-selected="false">IP Diet Report</button>
                    </li>
                </ul>
                </div>
                <div class="tab-content" id="pills-tabContent">
                    <div class="tab-pane fade show active" id="pills-opdiet" role="tabpanel" aria-labelledby="pills-opdiet-tab">
                            ...
                            <div class="container-fluid mainContents">
                                <div class="card card-body main_card mt-2 p-3 mb-2">                           
                                    <div class="main_heading  mb-2 p-2">
                                        <div class="container-fluid ">
                                        <form class="AddForm" id="diet_teport" novalidate>
                                            <div class="row">
                                                    <div class="col-xl-2 col-lg-2 col-6">
                                                        <label class="mt-3 mb-1 inputlabel ">Op Number<span  style="color:red"> *</span></label><br>
                                                        <select class="SearchAndSelect inputfield op_no" aria-label="Default select example name"id="op_no" name="OPNo" required>
                                            ...
                                            <div class="row mt-3 d-flex justify-content-between">
                                                <p><b>Name : </b><span id="name-value"></span></p>
                                                <p class="mb-0"><b>Age : </b><span id="age-value"></span></p>
                                            </div>         
                                            <div class=" col-12 text-end fs-4">
                                                <i id="PrintOpDiet" class="ri-printer-line  reportprinticon"></i>
                                            </div>                          
                                        </div>
                                    </div>
                                    <div>
                    ...
                    <div class="tab-pane fade" id="pills-ipdiet" role="tabpanel" aria-labelledby="pills-ipdiet-tab">
                        ...
                        <div class="container-fluid mainContents">
                            <div class="card card-body main_card mt-2 p-3 mb-2">                           
                                <div class="main_heading  mb-2 p-2">
                                    <div class="container-fluid ">
                                        <form class="AddForm" id="ip_diet_teport" novalidate>
                                            <div class="row">
                                                <div class="col-xl-2 col-lg-2 col-6">
                                        ...
                                        <div class="col-12 text-end fs-4">
                                            <i id="PrintIpDiet" class="ri-printer-line  reportprinticon"></i>
                                        </div>                             
                                    </div>
                                </div>
                                <div>
        ...
        <script type="text/javascript">
            ...
            $(document).on('submit', '#diet_teport', function(e) {
                e.preventDefault();
                var OpNumb = $('#op_no').val();
                var ConNum = $('#consult_no').val();
                var Date = $('#date').val();
                var Count = $('#ip_datecount').val();

                var settings = {
                    "url": "/api/OpDietreportdata?OpNumber=" + OpNumb + "&ConNumber=" + ConNum + "&Date=" + Date + "&DataCount=" + Count,
                    "method": "GET",
                    "timeout": 0,
                    "headers": {
                ...
                $.ajax(settings).done(function(response) {
                    var OPdietreports = response.OPdietreports;
                    OPdietreports = OPdietreports.OPdietreports;
                    console.log(OPdietreports);

                    // Display name values from the first array
                    var firstReport = OPdietreports     [0];
                    ...
                    $.each(OPdietreports    , function(index, report) {
                        var row = $('<tr></tr>');
                        row.append('<td class="text-center">' + (index + 1) + '</td>');
                        row.append('<td class="text-center">' + report.date + '</td>');
                        row.append('<td class="text-center">' + report.time + '</td>');
                        row.append('<td class="text-center">' + report.Dietnames + '</td>');
                        row.append('<td class="text-center">' + report.diet_cost + '</td>');

                        tableBody.append(row);

                        totalAmount += parseFloat(report.diet_cost); // Accumulate treatment cost
                    });
            ...
            $(document).on('click', '#PrintOpDiet', function(e) {
                var OpNumb = $('#op_no').val();
                var ConNum = $('#consult_no').val();
                var Date = $('#date').val();
                var Count = $('#datecount').val();

                var url = 'OpDietPrint?OpNumber=' + OpNumb + '&ConNumber=' + ConNum + '&Date=' + Date + '&DataCount=' + Count;
        ...
        // ***************************************IP Report**********************************************************************//
            ...
            $(document).on('submit', '#ip_diet_teport', function(e) {
                e.preventDefault();
                ...
                var settings = {
                    "url": "/api/IpDietreportdata?IpNumber=" + IpNumb + "&ConNumber=" + IpConNum + "&Date=" + IpDate + "&DataCount=" + IpCount,
                    "method": "GET",
                    "timeout": 0,
                    "headers": {
                        "Accept": "application/json"
                ...
                $.ajax(settings).done(function(response) {
                    ...
                    IPdietreports = IPdietreports.IPdietreports;
                    ...
                    $.each(IPdietreports, function(index, report) {
                        var row = $('<tr></tr>');
                        row.append('<td class="text-center">' + (index + 1) + '</td>');
                        row.append('<td class="text-center">' + report.date + '</td>');
                        row.append('<td class="text-center">' + report.time + '</td>');
            ...
            $(document).on('click', '#PrintIpDiet', function(e) {
                var IpNumb = $('#ip_no').val();
                var IpConNum = $('#ip_consult_no').val();
                var IpDate = $('#ip_date').val();
                var IpCount = $('#ip_datecount').val();

                var url = 'IpdietPrint?IpNumber=' + IpNumb + '&ConNumber=' + IpConNum + '&Date=' + IpDate + '&DataCount=' + IpCount;
                window.open(url, '_blank');
            });
