    ...
    <body>
        ...
        <main id="main" class="main">
            <div class="container-fluid">
                <h4 class="d-flex justify-content-center py-1 contitle">IP Patient Records Edit</h4>   
            </div>        
                                ...
                                <div class="row">
                                        <div class="col-xl-6 col-lg-6 col-12">
                                            <div class="12">
                                                ...
                                                <select class="form-select inputfield IpNo" aria-label="Default select example name"id="ip_no" name="IpNo" disabled>
                                            @foreach ($ipno as $key )   
                                                    <option class="inputlabel" value="{{$key->IPP}}">{{$key->ip_no}} </option>
                                                   
                                                </select>
                                            </div>
                                            <div class="12">
                                                <label class="mt-2 mb-1 inputlabel">Date</label><br>
                                                <input type="date" class="form-control mt-1 inputfield Date" id="date" name="Date" value="{{$key->date}}"  required>
                                            </div>
                                            <div class="12">
                                            ...
                                            @foreach ($gpe as $genexam )
                                                @if ($genexam->input_type == 'check')
                                                    @foreach($jsonresult as $jsonresults)
                                                        ...
                                                        @endif
                                                    @endforeach
                                                    <input class="form-check-input patientrec" type="checkbox" value="" id="check{{$genexam->id}}" name="{{$genexam->name}}" data-records="{{$genexam->id}}" @if($Value == '1') checked @endif>                                      
                                        ...
                                        <div class="col-xl-6 col-lg-6 col-12">
                                            ...
                                            <label class="mt-3 mb-1 inputlabel">{{$generalexam->field_name}} </label><br>
                                                @foreach($jsonresult as $jsonresults)
                                                    ...
                                                    @endif
                                                @endforeach
                                            <textarea cols="30" rows="1" class="form-control mt-1 inputfield patientrec" id="{{$generalexam->name}}" name="{{$generalexam->name}}" data-records="{{$generalexam->id}}" >{{$Value}}</textarea>
                                            @endif
                                        </div>
                                        @endforeach 
                                    </div>
                                        ...
                                        <div class="col-xl-6 col-lg-6 col-12">
                                            ...
                                            <label class="mt-3 mb-1 inputlabel">{{$initialdetail->field_name}} </label><br>
                                                @foreach($jsonresult as $jsonresults)
                                                    ...
                                                    @endif
                                                @endforeach
                                            <textarea cols="30" rows="1" class="form-control mt-1 inputfield patientrec" id="{{$initialdetail->name}}" name="{{$initialdetail->name}}" data-records="{{$initialdetail->id}}" >{{$Value}}</textarea>
                                            @endif
                                        </div>
                                        @endforeach                                       
                                    </div>
                                        ...
                                        <div class="col-xl-6 col-lg-6 col-12">
                                            ...
                                            <label class="mt-3 mb-1 inputlabel">{{$sysexam->field_name}} </label><br>
                                                @foreach($jsonresult as $jsonresults)
                                                    ...
                                                    @endif
                                                @endforeach
                                            <textarea cols="30" rows="1" class="form-control mt-1 inputfield patientrec" id="{{$sysexam->name}}" name="{{$sysexam->name}}" data-records="{{$sysexam->id}}" >{{$Value}}</textarea>
                                            @endif
                                        </div>
                                        @endforeach  
                                    </div>
            ...
            $(document).ready(function() {
                ...
                $("#patient_record").validate({
                    submitHandler: function(form) {
                        ...
                        var UpdateId = $('#update_record_id').val();                
                        var Ipno_id = $('#ip_no').val();
                        var Date = $('#date').val();
                        $.ajax({
                            url: "/api/ippatient/"+UpdateId+"",
                            method: "PUT",
                            timeout: 0,
                            headers: {
                            ...
                            location.replace("{{ url('/IpPatientRecordView') }}")
                        });                               
                    }
                });
            });
