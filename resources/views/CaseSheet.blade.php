    ...
    <body>
                ...
                <div class="container-fluid mainContents">
                    <form id="case_sheet" class="AddForm">                                                                 
                        <div class="row mx-2">
                            ...
                            <div class="col-xl-4 col-lg-4 col-6">
                                ...
                                <select class="form-select inputfield op_no" aria-label="Default select example name"id="op_no" name="OPNo" >
                    ...
                    <form id="update_case_sheet" class="UpdateForm" style="display: none;">                                                                 
                        <div class="row mx-2">
                            ...
                            <div class="col-xl-4 col-lg-4 col-6">
                                <label class="mb-1 inputlabel">Op Number<span  style="color:red"> *</span></label><br>
                                <select class="form-select inputfield op_no" aria-label="Default select example name"id="update_op_no" name="UpdateOPNo" >
                                <option value=""> Choose OP Number</option>
                                @foreach ($opno as $key )   
                                    <option class="inputlabel" value="{{$key->id}}" data-test="Hello" data-gender="{{$key->gender}}">{{$key->op_no}} </option>
                                @endforeach    
                                </select>
                            </div>
                            <div class="col-xl-4 col-lg-4 col-6">
        ...
        <script type="text/javascript">
            ...
            var MasterTable = $('.MasterTable').DataTable({
                ...
                ajax: "{{ route('CaseSheet.casesheet') }}",
                columns: [
            ...
            //             var opId = value;
            //             var selectize = this;
            //             var selectedOption = selectize.getOption(value);
            //             var opGender = $(selectedOption).data('test');
            //             console.log(opGender);
            //             //var opGender = $('[data-value="' + value + '"]')[0].getAttribute('data-gender');
            //             loadOP(opId);
            ...
            $(document).on('change','.op_no',(function(){
                var opId = $(this).val();
                var opGender = $(this).find(':selected').data('gender');
                loadOP(opId);
                loadTherapists(opGender);
                showopdetails(opId);
                // console.log(opGender);
            ...
            function loadOP(OpNum) {
                if (OpNum) {
                    $.ajax({
                        url: '/api/getoplastConsultation/' + OpNum,
                        type: 'GET',
                        dataType: 'json',
                        success: function(data) {
                            $('.consult_no').empty();
                            if (data && data.consultation_no) {
                                $('.consult_no').append('<option class="inputlabel" value="' + data.id + '">' + data.consultation_no + '</option>');
                            } else {
